...
// jakiœ algorytm wykrywaj¹cy przeciêcie dwóch linii z internetu
...
// kolizjê wykrywamy gdy linia ³¹cz¹ca obecn¹ pozycjê z now¹ pozycj¹ przecina siê z któr¹œ z linii tworz¹cych œciany 
bool check_collision_with_wall(Point A, Point B, float* wall) {
	return intersection(A, B, { wall[0], wall[2] }, { wall[9], wall[11] });
}
// sprawdzamy kolizjê z ka¿d¹ ze œcian
...
int main(int argc, char** argv)
{
	...
	while (true)
	{
		if (SDL_PollEvent(&windowEvent))
		{
			...
			if (windowEvent.type == SDL_KEYDOWN)
			{
				...
				case SDLK_DOWN:
					...
					// // ¿eby wizualnie nie wchodziæ w œcianê - bez tego kolizja by³aby wykryta kiedy koordynaty œrodka
					// wejd¹ w œcianê, a wtedy widzimy juz to co jest za œcian¹. 
					...
					break;
				case SDLK_UP:
					// position temp zwiêkszamy (x o cos, z o sin)
					// // ¿eby wizualnie nie wchodziæ w œcianê - bez tego kolizja by³aby wykryta kiedy koordynaty œrodka
					// wejd¹ w œcianê, a wtedy widzimy juz to co jest za œcian¹. 
					position_temp = position + direction * glm::vec3(dp);
					if (!check_collision({ position.x, position.z }, { position_temp.x + cos(angle), position_temp.z + sin(angle) }))
		...
		// strza³kê rysujemy jako trójk¹t wpisany w okr¹g o œrodku w pozycji kamery 
